...
#' from that \eqn{\lambda}, else sets the starting \eqn{\lambda = \|x^T y\|_{\infty}/}(\code{nobs} \times \eqn{Var(y)})
...
#' @param verbose Logical input, default \code{FALSE}; if \code{TRUE}
...
#' identified in the final distribution of \eqn{\ell_1} norm of partial residuals).
...
#' \item{beta_mat}{A (\code{no_of_iter} + 1) \eqn{ \times} \code{nvars} matrix
...
#' \item{lambda0}{ Value of \eqn{\lambda_0} used in the Autotune LASSO.
...
#' \item{support_set}{Final set of predictors included in the support set by autotune lasso.}
#' \item{no_of_iter}{Number of iterations of coordinate descent performe
#' before noise variance estimate \eqn{\hat{\sigma}^2} converged.}
#' \item{no_of_beta_iter}{After noise variance estimate \eqn{\hat{\sigma}^2} has
