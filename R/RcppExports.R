...
#' Autotune LASSO with \eqn{\ell_2} norm of partial residuals: Fitting
#'  a linear model with fast and automatic lasso regularization
#' @aliases Autotune_l2
#' @description Fits a linear model via penalized maximum likelihood-inspired approach. Autotune
...
#' @param lambda Numeric input, default (and recommended) value None.
...
#'
#' @param beta_iter_max Maximum number of iterations of coordinate descent
...
#' residuals)
#' @return A list with various intermediate and final outputs
...
#' Autotune LASSO with active set selection: Fitting
#'  a linear model with fast and automatic lasso
#'  regularization
#' @aliases Autotune_active
#' @description Fits a linear model via penalized maximum likelihood-inspired approach. Autotune
#' LASSO's regularization path quickly picks out a good lambda for LASSO and then
#' returns the corresponding linear fit along with various attributes related to the
#' fit. It also incorporates active set selection insides the
#' standard autotune LASSO which is performed by fuction
#' \code{autotune_lasso}.
#'
#' @param x Matrix of predictors, with one column for each predictor.
#' Dimension will be \code{nobs} \eqn{\times} \code{nvars}; so each row is a new observation.
...
#' number of iterations of coordinate descent when sigma was not updating,
#' and the size of the estimated support set(or the number of outliers
#' identified in the final distribution of \eqn{\ell_2} norm of partial
#' residuals)
#' @return A list with various intermediate and final outputs
#'  produced by Autotune LASSO in its regularization path.
#'  \item{resi_mat}{A (\code{no_of_iter} + 1)\eqn{ \times nobs} matrix
