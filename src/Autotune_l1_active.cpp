...
// [[Rcpp::export]]
List autotune_lasso_active(NumericMatrix x,
                           NumericVector y,
                           float alpha = 0.01,
  ...
  NumericMatrix resi_mat(iter_max + beta_iter_max, n);
  NumericMatrix beta_mat(iter_max + beta_iter_max, p);
  ...
  while (error > tolerance && iteration <= iter_max) {
    ...
    sigma2est = ((n - 1) / std::max(n - iter - 1, 2)) * var(ytemp);
  ...
  if(support_set.size() == 0) {
    ...
    support_set = clone(old_support_set);
  ...
  while( beta_iteration < 2) {
    ...
    beta_mat(iteration + beta_iteration - 1, _) = beta;
    resi_mat(iteration + beta_iteration - 1, _) = r;
  ...
  act_pred_count = act_pred_count[Range(0, beta_iteration - 1)];
  resi_mat = resi_mat(Range(0, iteration + beta_iteration - 1), _);
  beta_mat = beta_mat(Range(0, iteration + beta_iteration - 1), _);
  ...
  if (verbose) {
    ...
    Rprintf("Out of %i predictors, %i were in active set.\n", p, active_set.size());
  }
  ...
  return List::create(
    ...
    _["active_pred_counts"] = act_pred_count,
